...
from utils import pubchem2smiles_batch
...
def main():
    ...
    PATH_UNI_EMB = 'Data/Raw/UniprotEmbeddings/human-per-protein.h5'
    id2embedding = {}

    with h5py.File(PATH_UNI_EMB, "r") as file:
        print(f"number of entries: {len(file.items())}")
        for sequence_id, embedding in tqdm(file.items(), desc='Retrieving Protein Embeddings'):
            id2embedding[sequence_id] = np.array(embedding)


    protein_feaures['embedding'] = protein_feaures.Uniprot.map(id2embedding)
    ...
    protein_feaures_matrix = pd.concat([protein_feaures, protein_feaures.embedding.apply(pd.Series)], axis=1).drop(columns= ['Uniprot', 'embedding'])
    protein_feaures_sparse = csr_matrix(protein_feaures_matrix.values)
